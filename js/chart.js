...
(function (scope) {
    ...
    scope.chart = function (data, config) {
        ...
        config.width = 1250;
        config.height = 1250;
        config.margin = config.margin || 225;
        ...
        config.initialAngle.arc = config.initialAngle.arc || {
            startAngle: 0,
            endAngle: aLittleBit
        };
        config.initialAngle.chord = config.initialAngle.chord || {
            source: config.initialAngle.arc,
            target: config.initialAngle.arc
        };
        ...
        function groupInfo(d) {
            var el = this;
            var line1 = data.names[d.id];
            var line2 = 'Total In: ' + formatNumber(Math.round(d.inflow));
            var line3 = 'Total Out: ' + formatNumber(Math.round(d.outflow));
            var color = d3.select(el).style('fill');
            d3.select('#line1').text(line1);
            d3.select('#line2').text(line2);
            d3.select('#line3').text(line3);
            d3.select('.info-tab').style('color', color);
            ...
            infoTimer = setTimeout(function () {
                    ...
                    var text = info.select('.text').selectAll('text')
                        .data([
            data.names[d.id],
            'Total In: ' + formatNumber(Math.round(d.inflow)),
            'Total Out: ' + formatNumber(Math.round(d.outflow))
            ]);
        ...
        function chordInfo(d) {
            ...
            var line1 = data.names[d.source.id];
            var line2 = 'To ' + data.names[d.target.id];
            var line3 = 'Total: ' + formatNumber(Math.round(d.source.value));
            var color = d3.select(el).style('fill');
            d3.select('#line1').text(line1);
            d3.select('#line2').text(line2);
            d3.select('#line3').text(line3);
        ...
        function draw(year, countries) {
            ...
            groupTextGroup
                ...
                .attrTween("transform", function (d) {
                    var i = d3.interpolate(previous.groups[d.id] || previous.groups[d.region] || meltPreviousGroupArc(d) || {
                        angle: 0
                    }, d);
                    return function (t) {
            ...
            groupTextGroup.exit()
                ...
                .attrTween("transform", function (d) {
                    ...
                    var region = layout.groups().filter(function (g) {
            ...
            groupTextPathPath.enter()
                ...
                .attr("id", function (d, i, k) {
                    return "group-textpath-arc" + d.id;
                });
            groupTextPathPath
                .style("fill", 'none')
                .transition()
            ...
            groupTextPath
                ...
                .attr("xlink:href", function (d, i, k) {
                    return "#group-textpath-arc" + d.id;
                });
